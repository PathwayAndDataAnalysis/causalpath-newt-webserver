...
var ChemicalView = Backbone.View.extend({
    ...
    format: function () {
        ...
        if (this.model.description[0] == undefined) this.$el.find('.chemical-description').hide();
    ...
    render: function () {
        ...
        $(document)
            ...
            .on('click', '#save-layout', function (evt) {
                ...
                currentLayoutProperties.packComponents = document.getElementById('pack-components')
                    .checked
                    ? true
                    : false;
...
var MapTabGeneralPanel = GeneralPropertiesParentView.extend({
    initialize: function () {
        ...
        $(document).on('change', '#map-type', function (evt) {
            ...
            if (
                (currentMapType == 'PD' || currentMapType == 'AF' || currentMapType == 'SIF') &&
        ...
        $(document).on('change', '#allow-compound-node-resize', function (evt) {
            ...
            self.params.allowCompoundNodeResize.value = $('#allow-compound-node-resize').prop(
                'checked'
            );
        ...
        $(document).on('change', '#enable-sif-topology-grouping', function (evt) {
            ...
            self.params.enableSIFTopologyGrouping.value = $('#enable-sif-topology-grouping').prop(
                'checked'
            );
...
var MapTabRearrangementPanel = GeneralPropertiesParentView.extend({
    initialize: function () {
        ...
        $(document).on('change', '#animate-on-drawing-changes', function (evt) {
            ...
            self.params.animateOnDrawingChanges.value = $('#animate-on-drawing-changes').prop(
                'checked'
            );
    ...
    render: function () {
        ...
        $(document)
            ...
            .on('click', '#save-query-pathsbyURI', function (evt) {
                ...
                self.currentQueryParameters.URI =
                    document.getElementById('query-pathsbyURI-URI').value;
            ...
            .on('click', '#file-save-accept', function (evt) {
                ...
                if (fileformat === 'sbgn' || fileformat === 'nwt') {
                    ...
                    chiseInstance.saveAsCellDesigner(filename, function () {
                        var promptFileConversionErrorView = new PromptFileConversionErrorView({
                    ...
                    chiseInstance.saveAsSbml(filename, function (data, errorMessage) {
                        var promptSbmlConversionErrorView = new PromptSbmlConversionErrorView({
            ...
            .on('click', '#load-user-preferences-accept', function (evt) {
                ...
                if (document.getElementById('load-user-prefrences-map-check').checked) {
                    if (typeof preferences.currentGeneralProperties !== 'undefined') {
                        ...
                        if (
                            typeof preferences.currentGeneralProperties.mapColorSchemeStyle !==
                            'undefined'
                        ) {
                            applyColorScheme = true;
                            defaultColorSchemeStyle =
                                    ...
                                    if (
                                        ...
                                        targetNodes.forEach(function (node) {
                                            var infoboxesIndices =
                                                node.data('statesandinfos').length;
    ...
    render: function () {
        ...
        $(document)
            ...
            .on('click', '#prompt-invalidQuery-confirm', function (evt) {
                $(self.el).modal('toggle');
                if (PCdialog == 'Neighborhood') appUtilities.neighborhoodQueryView.render();
                else if (PCdialog == 'PathsBetween') appUtilities.pathsBetweenQueryView.render();
                else if (PCdialog == 'PathsFromTo') appUtilities.pathsFromToQueryView.render();
                else if (PCdialog == 'CommonStream') appUtilities.commonStreamQueryView.render();
            });
    ...
    render: function (data, errorMessage) {
        ...
        $(document)
            ...
            .on('click', '#prompt-sbml-confirm', function (evt) {
                var userAgreed = document.getElementById('sbml-coversion-user-agree').checked
                    ? true
                    : false;
...
var ReactionTemplateView = Backbone.View.extend({
    ...
    createDropdownElement: function (listOfOptionNames, listOfCSSClasses, listOfValues) {
    ...
    createRowElement: function (listOfDropdownOptions) {
        ...
        const inputFieldElement = this.createInputFieldElement(inputFieldCSSClasses);
    ...
    addModificationForConversion: function () {
        ...
        const modificationSymbols = ['P', 'Ub', 'OH', 'Ac', 'G', 'My', 'Pa', 'Pr', 'S', 'Me', 'H'];
    ...
    getMetabolicReactionParameters: function () {
        const inputNames = $('#metabolic-reaction-input-table  :input.template-reaction-textbox')
            .map(function () {
                return $(this).val();
            })
            .toArray();
        ...
        const outputNames = $('#metabolic-reaction-output-table  :input.template-reaction-textbox')
            .map(function () {
                return $(this).val();
            })
            .toArray();
    ...
    getConversionParameters: function () {
        ...
        const macromoleculeMultimer = $('#conversion-macromolecule-multimer-checkbox').prop(
            'checked'
        );
        ...
        const conversionTypes = $('#reaction-top-input-row select.conversion-type-dropdown')
            .map(function () {
                return $(this).val();
            })
            .toArray();
    ...
    getMultimerizationParameters: function () {
        ...
        const hasRegulator = $('#multimerization-regulator-checkbox').prop('checked');
        ...
        self.bricksWithOptionalRegulator.forEach((type) => {
            ...
            $(document).on('click', regulatorCheckboxId, function (event) {
                ...
                hasRegulatorEdgeTypeSelector =
                    type === 'multimerization' || type === 'association' || type === 'dissociation';
            ...
            $(document).on('change', regulatorTypeInputId, function () {
                // determine if multimer should be enabled
                const selectedRegulatorType = $(regulatorTypeInputId).val();
                const multimerEnabled = self.isMultimerEnabledType(selectedRegulatorType);
                self.setMultimerCheckboxEnabled(type, multimerEnabled);
    ...
    render: function () {
        ...
        $(document)
            ...
            .on('click', '#save-grid', function (evt) {
                ...
                currentGridProperties.showDistributionGuidelines = document.getElementById(
                ...
                currentGridProperties.horizontalGuidelineColor = document.getElementById(
    ...
    options.forEach(function (option) {
        ...
        if (optionStyle && optionStyle.length > 0) {
            ...
            optionStyle.forEach(function (style) {
                var val = typeof style.value == 'function' ? style.value(optionVal) : optionVal;
...
var FontPropertiesView = Backbone.View.extend({
    ...
    extendProperties: function (eles) {
        ...
        var commonFontSize = parseInt(
            chiseInstance.elementUtilities.getCommonProperty(eles, 'font-size', 'data')
        );
