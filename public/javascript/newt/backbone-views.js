...
var LayoutPropertiesView = Backbone.View.extend({
	...
	getLayoutOptions: function (preferences, _chiseInstance) {
		...
		var chiseInstance =
			_chiseInstance || appUtilities.getActiveChiseInstance();
	...
	render: function () {
		...
		$(document)
			...
			.on('click', '#save-layout', function (evt) {
				...
				currentLayoutProperties.tile =
					document.getElementById('tile').checked;
				currentLayoutProperties.packComponents =
					document.getElementById('pack-components').checked
						? true
						: false;
...
var ColorSchemeInspectorView = Backbone.View.extend({
	initialize: function () {
		...
		$(document).on(
			'click',
			'div.color-scheme-invert-button',
			function (evt) {
				var raw_id = $(this).attr('id');
				var cy = appUtilities.getActiveCy();
				var scheme_type = appUtilities.getScratch(
		...
		$(document).on(
			'click',
			'#map-color-scheme-default-button',
			function (evt) {
				var cy = appUtilities.getActiveCy();
	...
	render: function () {
		this.template = _.template(
			$('#color-scheme-inspector-template').html()
		);
		var cy = appUtilities.getActiveCy();
...
var GeneralPropertiesParentView = Backbone.View.extend({
	...
	applyUpdate: function () {
		...
		chiseInstance.setShowComplexName(
			currentGeneralProperties.showComplexName
		);
...
var MapTabGeneralPanel = GeneralPropertiesParentView.extend({
	initialize: function () {
		...
		$(document).on('click', '#map-general-default-button', function (evt) {
			...
			actions.push({
				name: 'changeMenu',
				param: self.params.compoundPadding,
			});
			actions.push({ name: 'changeMenu', param: self.params.arrowScale });
			actions.push({
				name: 'changeCss',
			...
			actions.push({
				name: 'changeMenu',
				param: self.params.highlightColor,
			});
			ur.do('batch', actions);
		});
	},
...
var MapTabLabelPanel = GeneralPropertiesParentView.extend({
	initialize: function () {
		...
		self.params.adjustNodeLabelFontSizeAutomatically = {
			...
			type: 'checkbox',
			property:
				'currentGeneralProperties.adjustNodeLabelFontSizeAutomatically',
		};
		...
		$(document).on('click', '#map-label-default-button', function (evt) {
			...
			actions.push({
				name: 'changeMenu',
				param: self.params.dynamicLabelSize,
			});
			actions.push({
				name: 'changeMenu',
			...
			actions.push({
				name: 'changeMenu',
				param: self.params.fitLabelsToNodes,
			});
			actions.push({
				name: 'changeMenu',
			...
			actions.push({
				name: 'changeMenu',
				param: self.params.showComplexName,
			});
			ur.do('batch', actions);
		});
	},
...
var MapTabRearrangementPanel = GeneralPropertiesParentView.extend({
	initialize: function () {
		...
		self.params.rearrangeOnComplexityManagement = {
			...
			type: 'checkbox',
			property:
				'currentGeneralProperties.rearrangeOnComplexityManagement',
		};
	...
	initialize: function () {
		...
		function fillTable4ExpData(elem, data) {
			...
			for (let i = 0; i < nodeCnt; i++) {
				...
				$(document).on(
					'mouseenter',
					'#hoverable-table-row-' + i,
					() => {
						var cy = appUtilities.getActiveCy();
						var vu = cy.viewUtilities('get');
						var eles = cy
							.nodes()
							.filter((x) => x.data('label') == nodeLabels[i]);
						vu.highlight(eles, 2);
					}
				);
				$(document).on(
		...
		function searchTable(table, filter, searchIdxes) {
			...
			for (var i = 0; i < rows.length; i++) {
				for (var j = 0; j < searchIdxes.length; j++) {
					var col =
						rows[i].getElementsByTagName('td')[searchIdxes[0]];
					if (col) {
	...
	loadExperiment: function (params) {
		...
		for (var i = 0; i < panels.length; i++) {
			if (
				...
				panels[i].id !=
					'sbgn-inspector-map-properties-experiment-heading'
			) {
				$(panels[i]).click();
			}
		}
	...
	render: function () {
		...
		var refreshButtons = function (param) {
			...
			for (let i in visibleFiles) {
				...
				if (button != null) {
					if (
						fileTitle[i] != undefined &&
						fileDescription[i] != undefined
					) {
						button.title =
							button.title +
							i +
		...
		if (
			...
			Object.entries(currentGeneralProperties.experimentDescription)
				.length != 0
		) {
			//document.getElementById('sbgn-inspector-map-color-scheme').style.visibility = "hidden";
			document.getElementById(
				'sbgn-inspector-map-color-scheme'
			).style.display = 'none';
		} else {
			document.getElementById(
				'sbgn-inspector-map-color-scheme'
	...
	render: function () {
		...
		$(document)
			...
			.on('click', '#save-query-neighborhood', function (evt) {
				...
				self.currentQueryParameters.geneSymbols =
					document.getElementById(
						'query-neighborhood-gene-symbols'
					).value;
				self.currentQueryParameters.lengthLimit = Number(
					document.getElementById('query-neighborhood-length-limit')
						.value
				);

				var geneSymbols =
					self.currentQueryParameters.geneSymbols.trim();
				if (geneSymbols.length === 0) {
					document
						.getElementById('query-neighborhood-gene-symbols')
						.focus();
					return;
				}
				...
				if (self.currentQueryParameters.lengthLimit > 2) {
					...
					document
						.getElementById('query-neighborhood-length-limit')
						.focus();
					return;
				}
	...
	render: function () {
		...
		$(document)
			...
			.on('click', '#save-query-pathsbetween', function (evt) {
				...
				self.currentQueryParameters.lengthLimit = Number(
					document.getElementById('query-pathsbetween-length-limit')
						.value
				);
				...
				if (geneSymbols.length === 0) {
					document
						.getElementById('query-pathsbetween-gene-symbols')
						.focus();
					return;
				}
				// geneSymbols is cleaned up from undesired characters such as #,$,! etc. and spaces put before and after the string
				...
				if (self.currentQueryParameters.lengthLimit > 3) {
					...
					document
						.getElementById('query-pathsbetween-length-limit')
						.focus();
					return;
				}
	...
	render: function () {
		...
		$(document)
			...
			.on('click', '#save-query-pathsfromto', function (evt) {
				...
				self.currentQueryParameters.sourceSymbols =
					document.getElementById(
						'query-pathsfromto-source-symbols'
					).value;
				self.currentQueryParameters.targetSymbols =
					document.getElementById(
						'query-pathsfromto-target-symbols'
					).value;
				self.currentQueryParameters.lengthLimit = Number(
					document.getElementById('query-pathsfromto-length-limit')
						.value
				);

				var sourceSymbols =
					self.currentQueryParameters.sourceSymbols.trim();
				if (sourceSymbols.length === 0) {
					document
						.getElementById('query-pathsfromto-source-symbols')
						.focus();
					return;
				}
				...
				var targetSymbols =
					self.currentQueryParameters.targetSymbols.trim();
				if (targetSymbols.length === 0) {
					document
						.getElementById('query-pathsfromto-target-symbols')
						.focus();
					return;
				}
				...
				if (self.currentQueryParameters.lengthLimit > 3) {
					...
					document
						.getElementById('query-pathsfromto-length-limit')
						.focus();
					return;
				}
	...
	render: function () {
		...
		$(document)
			...
			.on('click', '#save-query-commonstream', function (evt) {
				...
				self.currentQueryParameters.lengthLimit = Number(
					document.getElementById('query-commonstream-length-limit')
						.value
				);
				...
				if (geneSymbols.length === 0) {
					document
						.getElementById('query-commonstream-gene-symbols')
						.focus();
					return;
				}
				// geneSymbols is cleaned up from undesired characters such as #,$,! etc. and spaces put before and after the string
				...
				if (self.currentQueryParameters.lengthLimit > 3) {
					...
					document
						.getElementById('query-commonstream-length-limit')
						.focus();
					return;
				}
	...
	render: function () {
		...
		$(document)
			...
			.on('click', '#save-query-pathsbyURI', function (evt) {
				...
				var queryURL =
					'http://www.pathwaycommons.org/pc2/get?uri=' +
					uri +
					'&format=SBGN';
				...
				if (fileformat === 'sbgn' || fileformat === 'nwt') {
					...
					if (fileformat === 'sbgn') {
						...
						nodes = cy.nodes().filter(function (node) {
							return !chiseInstance.elementUtilities.isSIFNode(
								node
							);
						});
						...
						edges = cy.edges().filter(function (edge) {
							return (
								!chiseInstance.elementUtilities.isSIFEdge(
									edge
								) &&
						...
						// get topologyGrouping instance for cy
						var topologyGrouping =
							chiseInstance.sifTopologyGrouping;
						var compoundGroups =
							topologyGrouping.getGroupCompounds();
					...
					chiseInstance.saveAsCellDesigner(filename, function () {
						...
						).innerText = 'Conversion service is not available!';
					});
				} else if (fileformat === 'sbml') {
					chiseInstance.saveAsSbml(
						filename,
					...
					console.error(
						'FileSaveView received unsupported file format: ' +
							fileformat
					);
				}
...
var SaveUserPreferencesView = Backbone.View.extend({
	...
	render: function () {
		...
		$('#save-user-prefrences-object-check')
			...
			.on('change', function () {
				if (
					document.getElementById('save-user-prefrences-object-check')
						.checked
				) {
					$('.save-preferences-object-styles').prop('checked', true);
					$('.save-preferences-object-styles').attr(
						'disabled',
						'disabled'
					);
				} else {
		...
		$(document)
			...
			.on('click', '#save-user-preferences-accept', function (evt) {
				...
				//get grid properties
				if (
					document.getElementById('user-prefrences-grid-check')
						.checked
				) {
				...
				// get currentGeneralProperties for cy
				if (
					document.getElementById('user-prefrences-map-check').checked
				) {
					...
					Object.keys(currentGeneralProperties).forEach(function (
						key,
						index
					) {
						if (
				...
				if (
					document.getElementById('user-prefrences-layout-check')
						.checked
				) {
					...
					Object.keys(currentLayoutProperties).forEach(function (
						key,
						index
					) {
				...
				if (typeof appUtilities.stagedElementStyles !== 'undefined') {
					appUtilities.stagedElementStyles.forEach(function (
						element
					) {
						if (
							document.getElementById(
								'user-prefrences-object-' +
									element['element'] +
									'-check'
							).checked
						) {
...
var LoadUserPreferencesView = Backbone.View.extend({
	...
	render: function (param) {
		...
		$('#load-user-prefrences-object-check')
			...
			.on('change', function () {
				if (
					document.getElementById('load-user-prefrences-object-check')
						.checked
				) {
					$('.load-preferences-object-styles').prop('checked', true);
					$('.load-preferences-object-styles').attr(
						'disabled',
						'disabled'
					);
				} else {
		...
		$(document)
			...
			.on('click', '#load-user-preferences-accept', function (evt) {
				...
				if (
					document.getElementById('load-user-prefrences-grid-check')
						.checked
				) {
					if (
						typeof preferences.currentGridProperties !== 'undefined'
					) {
						var currentGridProperties = appUtilities.getScratch(
							cy,
							'currentGridProperties'
						...
						$.extend(
							currentGridProperties,
							preferences.currentGridProperties
						);
						appUtilities.setScratch(
							cy,
							currentGridProperties,
						...
						cy.gridGuide({
							...
							geometricGuideline:
								currentGridProperties.showGeometricGuidelines,
							distributionGuidelines:
								currentGridProperties.showDistributionGuidelines,
							initPosAlignment:
								currentGridProperties.showInitPosAlignment,
							guidelinesTolerance:
								currentGridProperties.guidelineTolerance,
							guidelinesStyle: {
								...
								strokeStyle:
									currentGridProperties.guidelineColor,
								horizontalDistColor:
									currentGridProperties.horizontalGuidelineColor,
								verticalDistColor:
									currentGridProperties.verticalGuidelineColor,
				...
				if (
					document.getElementById('load-user-prefrences-layout-check')
						.checked
				) {
					if (
						typeof preferences.currentLayoutProperties !==
						'undefined'
					) {
						var currentLayoutProperties = appUtilities.getScratch(
							cy,
				...
				if (
					document.getElementById('load-user-prefrences-map-check')
						.checked
				) {
					if (
						...
						Object.keys(mapTabGeneralPanel.params).forEach(
							function (key, index) {
								if (
									typeof preferences.currentGeneralProperties[
										key
						...
						Object.keys(mapTabLabelPanel.params).forEach(function (
							key,
							index
						) {
							if (
						...
						Object.keys(mapTabRearrangementPanel.params).forEach(
							function (key, index) {
								if (
									typeof preferences.currentGeneralProperties[
										key
									...
									actions.push({
										name: 'changeMenu',
										param: mapTabRearrangementPanel.params[
											key
										],
									});
								}
				...
				if (typeof preferences.elementsStyles !== 'undefined') {
					preferences.elementsStyles.forEach(function (item, index) {
						...
						if (
							document.getElementById(
								'load-user-prefrences-object-' +
									sbgnClass +
									'-check'
							).checked
						) {
							var targetNodes = cy.elements(
								'[class="' + sbgnClass + '"]'
							);
								...
								if (
									...
									if (item['type'] == 'node') {
										...
										targetNodes.forEach(function (node) {
											...
											cy.trigger('noderesize.resizeend', [
												null,
												node,
											]);
										});
										...
										data['color'] =
											nameToValue['color'] != ''
												? nameToValue['color']
												: undefined;
										chiseInstance.changeFontProperties(
											targetNodes,
											data
										);
...
var PromptInvalidLengthLimitView = Backbone.View.extend({
	initialize: function () {
		var self = this;
		self.template = _.template(
			$('#prompt-invalidLengthLimit-template').html()
		);
	},
	render: function () {
		var self = this;
	...
	render: function (data, errorMessage) {
		...
		$(document)
			...
			.on('click', '#prompt-sbml-confirm', function (evt) {
				var userAgreed = document.getElementById(
					'sbml-coversion-user-agree'
				).checked
					? true
					: false;
...
var ReactionTemplateView = Backbone.View.extend({
	addMacromolecule: function (type, i) {
		...
		if (type == 'reaction') {
			html +=
				"<select class='template-reaction-molecule-type sbgn-output-medium layout-text' name='" +
				i +
				"' style='width: 120px'>" +
				"<option value='Macromolecule' selected> Macromolecule </option>" +
				"<option value='Simple Chemical'> Simple Chemical </option></td>";
			html +=
				"<td><img style='vertical-align: text-bottom;' class='template-reaction-delete-button' width='16px' height='16px' name='" +
				i +
				"' src='img/toolbar/delete-simple.svg'style='margin-right: 30px'/></td></tr>";
			$(
				'#template-reaction-dissociated-table :input.template-reaction-textbox'
			)
				.last()
				.closest('tr')
				.after(html);
			...
			html +=
				"<td><img style='vertical-align: text-bottom;' class='template-reversible-input-delete-button' width='16px' height='16px' name='" +
				i +
				"' src='img/toolbar/delete-simple.svg' style='margin-right: 30px'/></td></tr>";
			$(
				'#template-reversible-input-table :input.template-reaction-textbox'
			)
				.last()
				.closest('tr')
				.after(html);
			...
			html +=
				...
				"<option value='Simple Chemical' selected> Simple Chemical </option></td>";
			html +=
				"<td><img style='vertical-align: text-bottom;' class='template-reversible-output-delete-button' width='16px' height='16px' name='" +
				i +
				"' src='img/toolbar/delete-simple.svg'/></td></tr>";
			$(
				'#template-reversible-output-table :input.template-reaction-textbox'
			)
				.last()
				.closest('tr')
				.after(html);
	...
	removeMacromolecule: function (type, i) {
		if (type == 'reaction') {
			$(
				'#template-reaction-dissociated-table :input.template-reaction-textbox[name="' +
					i +
					'"]'
			)
				.closest('tr')
			...
			$(
				'#template-reversible-input-table :input.template-reaction-textbox[name="' +
					i +
					'"]'
			)
				.closest('tr')
			...
			$(
				'#template-reversible-output-table :input.template-reaction-textbox[name="' +
					i +
					'"]'
			)
				.closest('tr')
	...
	switchInputOutput: function (e) {
		...
		if (e == 'association') {
			...
			$('#reaction-template-left-td').html(
				self.reversibleInputHTMLContent
			);
			$('#reaction-template-right-td').html(
				self.reversibleOutputHTMLContent
			);
	...
	getAllParameters: function () {
		...
		var templateReactionComplexName = $(
			'#template-reaction-complex-name'
		).val();
		var nodeNames = $(
			'#template-reaction-dissociated-table  :input.template-reaction-textbox'
		)
			...
			.toArray();
		var nodeTypes = $(
			'#template-reaction-dissociated-table  :input.template-reaction-molecule-type :selected'
		)
			.map(function () {
				return $(this).val();
			})
		...
		var reversibleInputNodeNames = $(
			...
			.toArray();
		var reversibleInputNodeTypes = $(
			'#template-reversible-input-table :input.template-reaction-molecule-type :selected'
		)
			.map(function () {
				return $(this).val();
			})
			.toArray();
		var reversibleOutputNodeNames = $(
			'#template-reversible-output-table :input.template-reaction-textbox'
		)
			.map(function () {
				return $(this).val();
			...
			.toArray();
		var reversibleOutputNodeTypes = $(
			'#template-reversible-output-table :input.template-reaction-molecule-type :selected'
		)
			.map(function () {
				return $(this).val();
			})
		...
		var templateReactionEnableComplexName =
			$.trim(templateReactionComplexName).length != 0;
		...
		return {
			...
			templateReactionEnableComplexName:
				templateReactionEnableComplexName,
		};
	},
	disableDeleteButtonStyle: function (type) {
		if (type == 'reaction') {
			...
			$('img.template-reversible-input-delete-button').css(
				'opacity',
				0.2
			);
			$('img.template-reversible-input-delete-button').css(
			...
			$('img.template-reversible-output-delete-button').css(
				'opacity',
				0.2
			);
			$('img.template-reversible-output-delete-button').css(
	...
	initialize: function () {
		...
		$(document).on(
			'click',
			'.template-reaction-delete-button',
			function (event) {
				if (
					$(
						'#template-reaction-dissociated-table :input.template-reaction-textbox'
		...
		$(document).on(
			'click',
			'#template-reversible-input-add-button',
			function (event) {
				var nextIndex =
					parseInt(
						$(
		...
		$(document).on(
			'click',
			'#template-reversible-output-add-button',
			function (event) {
				var nextIndex =
					parseInt(
						$(
		...
		$(document).on(
			'click',
			'.template-reversible-input-delete-button',
			function (event) {
				if (
					$(
						'#template-reversible-input-table :input.template-reaction-textbox'
		...
		$(document).on(
			'click',
			'.template-reversible-output-delete-button',
			function (event) {
				if (
					$(
						'#template-reversible-output-table :input.template-reaction-textbox'
		...
		$(document).on('click', '#create-template', function (evt) {
			...
			var complexName = params.templateReactionEnableComplexName
				? params.templateReactionComplexName
				: undefined;
			var tilingPaddingVertical = chiseInstance.calculatePaddings(
				currentLayoutProperties.tilingPaddingVertical
			);
			var tilingPaddingHorizontal = chiseInstance.calculatePaddings(
				currentLayoutProperties.tilingPaddingHorizontal
			);
			...
			chiseInstance.createTemplateReaction(
				templateType,
				nodeList,
				complexName,
				undefined,
	...
	render: function () {
		...
		self.reversibleInputHTMLContent = $(
			'#reversible-template-left-td'
		).html();
		self.reversibleOutputHTMLContent = $(
			'#reversible-template-right-td'
		).html();
		return this;
	},
});
	...
	render: function () {
		...
		$(document)
			...
			.on('click', '#save-grid', function (evt) {
				...
				currentGridProperties.showDistributionGuidelines =
					document.getElementById(
						'show-distribution-guidelines'
					).checked;
				currentGridProperties.showInitPosAlignment =
					document.getElementById('show-init-Pos-Alignment').checked;
				...
				currentGridProperties.verticalGuidelineColor =
					document.getElementById('vertical-guideline-color').value;
				currentGridProperties.initPosAlignmentColor =
					document.getElementById('init-Pos-Alignment-Color').value;
				...
				currentGridProperties.distributionAlignmentRange = Number(
					document.getElementById('distribution-alignment-range')
						.value
				);
	...
	options.forEach(function (option) {
		...
		if (optionStyle && optionStyle.length > 0) {
			...
			optionStyle.forEach(function (style) {
				var val =
					typeof style.value == 'function'
						? style.value(optionVal)
						: optionVal;
...
var InfoboxPropertiesView = Backbone.View.extend({
	...
	updateCurrentProperties: function (infobox) {
		...
		self.currentProperties.generateSetAsDefaultText = function () {
			...
			var parent = chiseInstance.classes
				.getAuxUnitClass(infobox)
				.getParent(infobox, cy);
			var classInfo = appUtilities.transformClassInfo(
				parent.data('class')
			);
			var infoboxInfoMap = {
	...
	render: function (node, index) {
		...
		$(self.el).html(self.template);
		colorPickerUtils.bindPicker2Input(
			'#infobox-properties-border-color',
			null
		);
		colorPickerUtils.bindPicker2Input(
		...
		function readInfoboxProps() {
			...
			for (prop in self.propsMap) {
				...
				if (inputTypes[mappedProp] == 'regular') {
					val = $(
						'#' + generateInputId(mappedProp, self.selectorPrefix)
					).val();
		...
		$(document)
			...
			.on('click', '#set-as-default-infobox-properties', function (evt) {
				...
				if (stagedElement) {
					var stagedElementInfoboxStyles =
						stagedElement.infoBoxStyles.find(
					...
					appUtilities.stagedElementStyles.push({
						...
						styles: [],
						infoBoxStyles: [
							{ clazz: infoboxObj.clazz, styles: infoboxStyle },
						],
					});
				}
			});
		...
		this.$el.html(
			this.template({
				...
				userDefinedProperties:
					this.model.constructor.userDefinedProperties,
