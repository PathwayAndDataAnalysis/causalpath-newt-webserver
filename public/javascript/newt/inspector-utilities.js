...
inspectorUtilities.updateInputBoxesFromSet = function (
	ele,
	fieldName,
	parentSelector,
	subId,
...
inspectorUtilities.fillInspectorStateAndInfos = function (nodes, stateAndInfos, width) {
	...
	function getInfoboxDetailsBtnHtml(index) {
		var html =
			"<label id='inspector-infobox-" +
			index +
			"' style='cursor: pointer;font: 10pt Helvetica; font-weight:bold !important; margin:5px !important;'>" +
			'...' +
			'</label>';
	...
	for (var i = 0; i < stateAndInfos.length; i++) {
		(function (i) {
			...
			if (state.clazz == 'state variable') {
				...
				var uioHtml =
					"<div><input type='text' id='inspector-unit-of-information-label" +
					i +
					"' class='inspector-input-box' style='width: " +
					total +
					"px;' value='" +
					sanitizeInfoboxVal(state.label.text) +
					"'/>";
...
inspectorUtilities.handleSBGNInspector = function () {
	...
	if (allNodes || allEdges) {
		...
		html +=
			"<div  class='panel-heading' data-toggle='collapse' data-target='#inspector-style-properties-toggle'><p class='panel-title accordion-toggle'>" +
			title +
			'</p></div>';
		...
		if (allNodes) {
			...
			var borderColor = chiseInstance.elementUtilities.getCommonProperty(
				selectedEles,
				'border-color',
				'data'
			);
			...
			if (
				chiseInstance.elementUtilities.trueForAllElements(
					selectedEles,
					chiseInstance.elementUtilities.canHaveSBGNLabel
				)
			) {
				html +=
					"<tr><td style='width: " +
			...
			if (selectedEles.filter(':parent').length < 1) {
				html +=
					...
					width +
					"px; text-align:right; padding-right: 5px;'>" +
					"<font class='sbgn-label-font'>Width</font>" +
					"</td><td style='padding-left: 5px;'>" +
					"<input id='inspector-node-width' class='inspector-input-box' type='number' min='0' style='width: " +
					buttonwidth +
					"px;'";
				...
				html +=
					...
					width +
					"px; text-align:right; padding-right: 5px;'>" +
					"<font class='sbgn-label-font'>Height</font>" +
					"</td><td style='padding-left: 5px;'>" +
					"<input id='inspector-node-height' class='inspector-input-box' type='number' min='0' style='width: " +
					buttonwidth +
					"px;'";
      ...
      <input id='inspector-fill-color' class='inspector-input-box' type='color' style='width: ${buttonwidth}px;' value='${backgroundColor}'/>
			...
			html +=
				...
				width +
				"px; text-align:right; padding-right: 5px;'>" +
				"<font class='sbgn-label-font'>Border Width</font>" +
				"</td><td style='padding-left: 5px;'>" +
				"<input id='inspector-border-width' class='inspector-input-box' type='number' min='0' style='width: " +
				buttonwidth +
				"px;'";
      ...
      <input id='inspector-background-opacity-val' class='inspector-input-box' type='number' value='${bgOpacity}' style="width: 50px; display: inline; margin-left: 5px;" />
			...
			if (
				...
				html +=
					...
					width +
					"px; text-align:right; padding-right: 5px;'>" +
					"<font class='sbgn-label-font'>Font</font>" +
					"</td><td style='padding-left: 5px;'>" +
					"<label id='inspector-font' style='cursor: pointer;width: " +
					buttonwidth +
					"px;'>" +
					'...' +
					'<label/>' +
			...
			if (selectedEles.length > 0) {
				...
				var removeBtn =
					"<img id='inspector-delete-bg' width='16px' height='16px' class='pointer-button' " +
					'style="' +
					display +
					'"' +
					"src='app/img/toolbar/delete-simple.svg'>";
				...
				var fitSelection =
					'<select id="inspector-fit-selector" style="margin-right: 3px; margin-bottom: 2px;' +
					display +
					'">' +
					options +
					'</select>';
				...
				html +=
					...
					width +
					"px; text-align:right; padding-right: 5px;'>" +
					"<font class='sbgn-label-font'>Image</font>" +
					"</td><td style='padding-left: 5px;'>" +
					"<div><button id='inspector-image-file' class='btn btn-default' style='width: " +
					width / 1.5 +
					"px;padding:2px;margin-bottom:2px;padding-bottom=0.5px;padding-top=0.5px;'>Choose...</button>" +
					"<input id='inspector-image-url' class='inspector-input-box' type='text' style='display: none; width: " +
					width / 1.5 +
					"px;' placeholder='Enter a URL...'/>" +
			...
			if (commonStateAndInfos) {
				if (
					chiseInstance.elementUtilities.trueForAllElements(
						selectedEles,
						chiseInstance.elementUtilities.canHaveStateVariable
					)
				) {
					...
					html +=
						...
						width +
						"px; text-align:right; padding-right: 5px;'>" +
						"<font class='sbgn-label-font'>State Variables</font>" +
						'</td>' +
						"<td id='inspector-state-variables' style='padding-left: 5px; width: '" +
						width +
						"'></td></tr>";
				}
				...
				if (chiseInstance.elementUtilities.canHaveUnitOfInformation(selectedEles)) {
					...
					html +=
						...
						width +
						"px; text-align:right; padding-right: 5px;'>" +
						"<font class='sbgn-label-font'>" +
						unit +
						' of Information</font>' +
						'</td>' +
						"<td id='inspector-unit-of-informations' style='padding-left: 5px; width: '" +
						width +
						"'></td></tr>";
				}
			}

			commonIsMultimer = chiseInstance.elementUtilities.getCommonProperty(
				selectedEles,
				function (ele) {
					return ele.data('class').endsWith(' multimer');
				}
			);
			commonIsCloned = chiseInstance.elementUtilities.getCommonProperty(
				selectedEles,
				function (ele) {
			...
			multimerCheck = chiseInstance.elementUtilities.trueForAllElements(
				selectedEles,
				chiseInstance.elementUtilities.canBeMultimer
			);
			clonedCheck = chiseInstance.elementUtilities.trueForAllElements(
				selectedEles,
				chiseInstance.elementUtilities.canBeCloned
			);
			...
			if (multimerCheck) {
				html +=
					...
					width +
					"px; text-align:right; padding-right: 5px;'>" +
					"<font class='sbgn-label-font'>Multimer</font>" +
					'</td>' +
					"<td style='padding-left: 5px; width: '" +
					width +
					"'><input type='checkbox' id='inspector-is-multimer'></td></tr>";
			}
			...
			if (clonedCheck) {
				html +=
					...
					width +
					"px; text-align:right; padding-right: 5px;'>" +
					"<font class='sbgn-label-font'>Cloned</font>" +
					'</td>' +
					"<td style='padding-left: 5px; width: '" +
					width +
					"'><input type='checkbox' id='inspector-is-clone-marker'></td></tr>";
			}
			...
			if (
				...
				html +=
					...
					width +
					"px; text-align:right; padding-right: 5px;'>" +
					"<font class='sbgn-label-font'>Ports</font>" +
					'</td>' +
					"<td style='padding-left: 5px; width: '" +
					width +
					"'>";
				...
				html +=
					"<select id='inspector-ports-ordering-select' class='input-medium layout-text' name='inspector-ports-ordering-select'>";
			...
			html +=
				...
				width +
				"px; text-align:right; padding-right: 5px;'>" +
				"<font class='sbgn-label-font'>Fill Color</font>" +
				"</td><td style='padding-left: 5px;'>" +
				"<input id='inspector-line-color' class='inspector-input-box' type='color' style='width: " +
				buttonwidth +
				"px;' value='" +
				commonLineColor +
				"'/>" +
			...
			html +=
				...
				"<font class='sbgn-label-font'>Width</font>" +
				"</td><td style='padding-left: 5px;'>" +
				"<input id='inspector-edge-width' class='inspector-input-box' type='number' min='0' style='width: " +
				buttonwidth +
				"px;'";
			if (commonLineWidth) {
			...
			if (
				...
				html +=
					...
					width +
					"px; text-align:right; padding-right: 5px;'>" +
					"<font class='sbgn-label-font'>PC IDs</font>" +
					'</td>' +
					"<td id='inspector-pc-ids' style='padding-left: 5px; width: '" +
					width +
					"'>" +
				...
				html +=
					...
					width +
					"px; text-align:right; padding-right: 5px;'>" +
					"<font class='sbgn-label-font'>Site Locations</font>" +
					'</td>' +
					"<td id='inspector-site-locations' style='padding-left: 5px; width: '" +
					width +
					"'>" +
		...
		if (selectedEles.length == 1) {
			...
			html +=
				"<div style='text-align: center; margin-top: 5px;'><button class='btn btn-default' style='align: center;' id='inspector-set-as-default-button'" +
				'>' +
				setAsDefaultTitle +
				'</button></div>';
		}
		...
		$('#sbgn-inspector-style-panel-group').append(
			'<div id="sbgn-inspector-style-properties-panel" class="panel" ></div>'
		);
		...
		if (type == 'node') {
			if (fillStateAndInfos) {
				inspectorUtilities.fillInspectorStateAndInfos(
					selectedEles,
					commonStateAndInfos,
					width
				);
				...
				$.ajax({
					...
					success: function (data) {
						...
						if (
							...
							typeof applyBackground === 'function'
						)
			...
			$('#inspector-image-url').on('change', function () {
				...
				if (url && imageURL !== url) {
					...
					if (oldObj !== undefined)
						...
						chiseInstance.addBackgroundImage(
							selectedEles,
							obj,
							updateBackgroundDeleteInfo,
							promptInvalidImage,
			...
			$('#inspector-is-multimer').on('click', function () {
				chiseInstance.setMultimerStatus(
					selectedEles,
					$('#inspector-is-multimer').prop('checked')
				);
			});
			...
			$('#inspector-is-clone-marker').on('click', function () {
				chiseInstance.setCloneMarkerStatus(
					selectedEles,
					$('#inspector-is-clone-marker').prop('checked')
				);
			});
			...
			$('#inspector-label').on('keydown', function (e) {
				...
				if (e.keyCode == 13 && e.shiftKey) {
					...
					var tmp =
						$(this).val().substring(0, cursor_position) +
						' \\n ' +
		...
		for (var i = 0; i < nodes.length; i++) {
			if (
				...
				if (groupEPN.includes(errorCode) && eles.target().data().id != nodes[i].data().id) {
					...
					errorCode == 'pd10110' &&
					chiseInstance.elementUtilities.isPNClass(nodes[i])
				)
					listedNodes.push(nodes[i]);
				else if (
					errorCode == 'pd10125' &&
	...
	if (
		...
		for (var i = 0; i < connectedEdges.length; i++) {
			...
			if (i == 0) {
				if (errorCode == 'pd10104')
					html +=
						'<p style="text-align:center" > To fix, choose a consumption glyph connected to the dissociation glyph: </p>  ';
				else if (errorCode == 'pd10108')
					html +=
						'<p style="text-align:center" > To fix, choose a production glyph connected to the association glyph: </p> ';
				else if (errorCode == 'pd10111')
					html +=
						'<p style="text-align:center" > To fix, choose an arcs whose source is ' +
						eles.data().class.toUpperCase() +
						':</p>  ';
				else if (errorCode == 'pd10126')
					html +=
						'<p style="text-align:center" > To fix, choose a logic arc connected to the logical operator: </p>  ';
				html +=
			...
			if (errorCode == 'pd10104')
				html +=
					'<label class="radio"><input type="radio" name="optpd10104" value="' +
					connectedEdges[i].source().data().label +
					'" checked>' +
					connectedEdges[i].source().data().label +
					' to dissociation </label>';
			else if (errorCode == 'pd10111')
				html +=
					'<label class="radio"><input type="radio" name="optpd10111" value="' +
					connectedEdges[i].target().id() +
					'" checked>' +
					eles.data().class.toUpperCase() +
					' to ' +
				...
				html +=
					'<label class="radio"><input  type="radio" name="optpd10126" value="' +
					connectedEdges[i].id() +
					'" checked>' +
					connectedEdges[i].source().data().label +
					' to ' +
				...
				html +=
					'<label class="radio"><input type="radio" name="optpd10108" value="' +
					connectedEdges[i].target().data().label +
					'" checked> Association to ' +
					connectedEdges[i].target().data().label +
					' </label>';
		}
		if (connectedEdges.length > 0) html += '</div>';
		else params.handled = false;
		...
		if (
			...
			html +=
				...
				'">';
			html +=
				'<label class="radio"><input type="radio" name="optpd10142" value="consumption"> consumption </label>';
			html +=
				'<label class="radio"><input type="radio" name="optpd10142" value="production" checked> production </label>';
		} else {
		...
		for (var i = 0; i < listedNodes.length; i++) {
			if (i == 0) {
				if (errorCode == 'pd10109')
					html +=
						'<p style="text-align:center" > To fix, choose a glyph of EPN class or a logical operator as a source reference to ' +
						eles.data().class +
						': </p>  ';
				else if (errorCode == 'pd10112')
					html +=
						'<p style="text-align:center" > To fix, choose a compartment to place top level glyph inside: </p>  ';
				else if (errorCode == 'pd10124')
					html +=
						'<p style="text-align:center" > To fix, choose a glyph of EPN class as a source reference to the logic arc: </p>  ';
				else if (errorCode == 'pd10125')
					html +=
						'<p style="text-align:center" > To fix, choose a logical operator as a target reference to the logic arc: </p>  ';
				else if (errorCode == 'pd10128')
					html +=
						'<p style="text-align:center" > To fix, choose a glyph of tag, submap or terminal class as a target reference to the equivalence arc: </p>  ';
				else if (errorCode == 'pd10127')
					html +=
						'<p style="text-align:center" > To fix, choose a glyph of EPN class as a source reference to the equivalence arc: </p>  ';
				else
					html +=
						'<p style="text-align:center" > To fix, choose a glyph of PN class as a target reference to the ' +
						eles.data().class +
						': </p> ';
				html +=
					'<div style="margin: 0 auto;width: auto;text-align: left; display: table;"class="radio validation-error-radio" id="errors' +
			...
			if (errorCode != 'pd10112') {
				...
				if (errorCode == 'pd10110' || errorCode == 'pd10128')
					html +=
						...
						listedNodes[i].id() +
						'" checked>' +
						listedNodes[i].data().class.charAt(0).toUpperCase() +
						listedNodes[i].data().class.slice(1) +
						' </label>';
				else if (errorCode == 'pd10125')
					html +=
						'<label class="radio"><input type="radio" name="optradio" value="' +
		...
		if (listedNodes.length > 0) html += '</div>';
		else params.handled = false;
	}
	return html;
};
...
inspectorUtilities.handleSBGNConsole = function (errors, currentPage, cy, data, notPD) {
	...
	if (errors.length != 0 && !notPD) {
		...
		if (errors[currentPage].pattern == 'pd10109' || errors[currentPage].pattern == 'pd10110') {
			...
			errors[currentPage].text[0] = errors[currentPage].text[0].replace(
				...
				elesClass.charAt(0).toUpperCase() + elesClass.slice(1)
			);
		}
		...
		if (errors[currentPage].pattern == 'pd10101') {
			...
			errors[currentPage].pattern == 'pd10105' ||
			errors[currentPage].pattern == 'pd10106'
		) {
			if (
				chiseInstance.elementUtilities.isPNClass(eles.target()) &&
				chiseInstance.elementUtilities.isEPNClass(eles.source())
		...
		if (currentPage == 0) {
			if (errors.length != 1) {
				html +=
					"<div id = 'altItems' style='text-align: center; margin-top: 5px; '><button class='btn btn-default' style='align: center;' id='inspector-next-button'" +
					'>' +
					next +
					"</button> <button class='btn btn-default' style='align: center;' id='inspector-dismiss-button'" +
					'>' +
					dismiss +
					'</button> </div>';
			} else {
				html +=
					"<div id = 'altItems' style='text-align: center; margin-top: 5px;  ' ><button class='btn btn-default' style='align: center;' id='inspector-dismiss-button'" +
					'>' +
					dismiss +
					'</button> </div>';
			}
			...
			if (currentPage + 1 !== errors.length) {
				html +=
					"<div id = 'altItems' style='text-align: center; margin-top: 5px;  ' >\n\
                   <button class='btn btn-default' style='align: center;' id='inspector-back-button'" +
					'>' +
					back +
					"</button> <button class='btn btn-default' style='align: center;' id='inspector-next-button'" +
					'>' +
					next +
					"</button> <button class='btn btn-default' style='align: center;' id='inspector-dismiss-button'" +
					'>' +
				...
				html +=
					"<div id = 'altItems' style='text-align: center; margin-top: 5px; ' ><button class='btn btn-default' style='align: center;' id='inspector-back-button'" +
					'>' +
					back +
					"</button> <button class='btn btn-default' style='align: center;' id='inspector-dismiss-button'" +
					'>' +
		...
		html +=
			'<b><p class=\'panel-body\' style="color:red; text-align:center" > Can only validate maps of type PD</p></b>';
		html +=
			"<div id = 'altItems' style='text-align: center; margin-top: 5px;  ' ><button class='btn btn-default' style='align: center;' id='inspector-dismiss-button'" +
			'>' +
			dismiss +
		...
		html +=
			'<b><p class=\'panel-body\' style="color:green; text-align:center" > Map is Valid</p></b>';
		html +=
			"<div id = 'altItems' style='text-align: center; margin-top: 5px;  ' ><button class='btn btn-default' style='align: center;' id='inspector-dismiss-button'" +
			'>' +
			dismiss +
	...
	$('#fix-errors-of-validation-icon').on('click', function () {
		...
		if (errors[currentPage].pattern == 'pd10101') {
			...
			errors[currentPage].pattern == 'pd10103' ||
			errors[currentPage].pattern == 'pd10107'
		) {
			...
			for (var i = 0; i < addedNodeNum; i++) {
				var edgeParams = {
					...
					language: edges[i].data().language,
				};
				var shiftX = 22;
				var shiftY = 22;
				...
				if (edges[i].data().class == 'consumption') {
					if (eles.position().x > target.position().x) shiftX *= -1;
					if (eles.position().y > target.position().y) shiftY *= -1;
				} else {
					if (eles.position().x > source.position().x) shiftX *= -1;
					if (eles.position().y > source.position().y) shiftY *= -1;
				}
				var cX = x + shiftX;
				var cY = y + shiftY;
			...
			var chiseInstance = appUtilities.getActiveChiseInstance();
			if (
				chiseInstance.elementUtilities.isPNClass(targetTmp) &&
				chiseInstance.elementUtilities.isEPNClass(sourceTmp)
			) {
				errorFixParam.edge = eles;
			}
		} else if (errors[currentPage].pattern == 'pd10140') {
	...
	$('input[type=radio]').change(function () {
		...
		if (errors[currentPage].pattern == 'pd10104') {
			...
			if (errors[currentPage].pattern == 'pd10124') {
				var zoomLevel = 4;
				if (zoomLevel < cy.zoom()) zoomLevel = cy.zoom();
				cy.animate({
					duration: 1400,
					center: { eles: node },
					easing: 'ease',
	...
	cy.animate({
		...
		complete: function () {
			...
			if (eles.isEdge()) {
				...
				var source_loc =
					Math.pow(source_node._private.position.x, 2) +
					Math.pow(source_node._private.position.y, 2);
				var target_loc =
					Math.pow(target_node._private.position.x, 2) +
					Math.pow(target_node._private.position.y, 2);
				...
				if (
					renderedEndPosition.x >= maxRenderedX ||
					renderedStartPosition.x <= 0 ||
					renderedEndPosition.y >= maxRenderedY ||
					renderedStartPosition.y <= 0
				) {
					exceed = true;
				}
				if (exceed) {
				...
				if (
					bottomRightRenderedPosition.x >= maxRenderedX ||
					topLeftRenderedPosition.x <= 0 ||
					bottomRightRenderedPosition.y >= maxRenderedY ||
					topLeftRenderedPosition.y <= 0
				) {
					exceed = true;
				}
				if (exceed) {
