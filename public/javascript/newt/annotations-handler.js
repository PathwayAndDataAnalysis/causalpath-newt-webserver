...
ns.vocabulary = {
	...
	'bqmodel:is': {
		...
		controlled: true,
		description:
			'The modelling object represented by the model element is identical with ' +
	...
	'bqmodel:isDerivedFrom': {
		...
		controlled: true,
		description:
			'The modelling object represented by the model element is derived from the ' +
	...
	'bqmodel:isDescribedBy': {
		...
		controlled: true,
		description:
			'The modelling object represented by the model element is described by ' +
	...
	'bqmodel:isInstanceOf': {
		...
		controlled: true,
		description:
			'The modelling object represented by the model element is an instance of ' +
	...
	'bqbiol:is': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element has identity with ' +
			'the subject of the referenced resource. This relation might be used to ' +
	...
	'bqbiol:encodes': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element encodes, directly ' +
	...
	'bqbiol:hasPart': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element includes the ' +
	...
	'bqbiol:hasProperty': {
		...
		controlled: true,
		description:
			'The subject of the referenced resource is a property of the biological ' +
	...
	'bqbiol:hasVersion': {
		...
		controlled: true,
		description:
			'The subject of the referenced resource is a version or an instance of ' +
	...
	'bqbiol:isDescribedBy': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element is described by ' +
	...
	'bqbiol:isEncodedBy': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element is encoded, ' +
	...
	'bqbiol:isHomologTo': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element is homologous to ' +
	...
	'bqbiol:isPartOf': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element is a physical or ' +
	...
	'bqbiol:isPropertyOf': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element is a property of ' +
	...
	'bqbiol:isVersionOf': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element is a version or ' +
	...
	'bqbiol:occursIn': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element is physically ' +
	...
	'bqbiol:hasTaxon': {
		...
		controlled: true,
		description:
			'The biological entity represented by the model element is taxonomically ' +
	...
	'sio:SIO_000223': {
		...
		controlled: false,
		description:
			'Attach a custom property to the entity. You can freely specify the name ' +
...
ns.fillAnnotationsContainer = function (element) {
	...
	if (element.data('annotationsView')) {
		...
		element.data('annotations') &&
		!$.isEmptyObject(element.data('annotations'))
	) {
		...
		for (var annotID in annotations) {
			...
			if (!ns.vocabulary[model.get('selectedRelation')].controlled) {
				if (
					!_.contains(
						ns.Annotation.userDefinedProperties,
						model.get('selectedDB')
					)
				) {
					ns.Annotation.userDefinedProperties.push(
						model.get('selectedDB')
					);
				}
			}
...
ns.validateAnnotation = function (dbKey, resourceID, callback) {
	...
	if (resourceID.startsWith(identifiersURL)) {
		...
		if (resourceID.startsWith(identifiersURL + ns.dbList[dbKey].id)) {
			...
			callback(
				new Error(
					'Selected database is different than the one provided in the URL'
				)
			);
		}
	} else {
...
ns.Annotation = Backbone.Model.extend(
	{
		...
		sync: function (method, model, options) {
			switch (method) {
				case 'delete':
					delete model.get('cyParent').data('annotations')[
						model.get('id')
					];
					break;
				case 'read':
					var annotationData = model
						.get('cyParent')
						.data('annotations')[model.get('id')];
					model.set('status', annotationData.status);
					model.set('selectedDB', annotationData.selectedDB);
					model.set(
						'selectedRelation',
						annotationData.selectedRelation
					);
					model.set(
						'annotationValue',
					...
					break;
				case 'update':
					model.get('cyParent').data('annotations')[model.get('id')] =
						model.toJSON();
					break;
				case 'create':
