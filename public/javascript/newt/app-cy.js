...
module.exports = function (chiseInstance) {
    ...
    function registerUndoRedoActions() {
        ...
        ur.action(
            'relocateInfoBoxes',
            appUndoActions.relocateInfoBoxes,
            appUndoActions.relocateInfoBoxes
        );
        ur.action(
    ...
    function cytoscapeExtensionsAndContextMenu() {
            ...
            },
            {
                ...
                onClickFunction: function (event) {
                    ...
                    if (consumptionEdges.length > 0) {
                        var ur = cy.undoRedo();
                        ur.do('convertIntoReversibleReaction', {
                            processId: cyTarget.id(),
                            collection: consumptionEdges,
                            mapType: 'HybridAny',
                        });
                    }
        ...
        cy.viewUtilities({
            highlightStyles: [
                ...
                {
                    node: {'border-color': '#bf0603', 'border-width': 3},
                    edge: {
                        'line-color': '#bf0603',
                ...
                {
                    node: {'border-color': '#d67614', 'border-width': 3},
                    edge: {
                        'line-color': '#d67614',
        ...
        cy.layoutUtilities({
            // TODO: Make it to calculate desiredAspectRatio based on the size of the container
            // desiredAspectRatio: $(cy.container()).width() / $(cy.container()).height(),
            desiredAspectRatio: 0.8975225225225226,
        });
        ...
        cy.nodeEditing({
            ...
            resizeToContentCueEnabled: function (node) {
                ...
                return (
                    !node.data('onLayout') &&
                    result &&
                    !chiseInstance.elementUtilities.isResizedToContent(node) &&
                    cy.zoom() > 0.5
                );
            },
            ...
            if (appUtilities.getScratch(cy, 'dragAndDropModeEnabled')) {
                ...
                if (appUtilities.ctrlKeyDown) {
                    ...
                    if (self != cy) {
                        ...
                        if (
                            !newParent.data('class').startsWith('complex') &&
                            newParent.data('class') != 'compartment' &&
                    ...
                    if (self == cy || (self != cy && mouseDownNode != self)) {
                        chiseInstance.changeParent(
                            nodes,
                            newParent,
                            pos.x - dragAndDropStartPosition.x,
        ...
        */
        ...
        cy.on('tapend', function (event, relPos) {
            ...
            if (modeProperties.mode === 'add-node-mode') {
                ...
                if (
                    ...
                    chiseInstance.addProcessWithConvenientEdges(
                        convenientProcessSource,
                        cyTarget,
                        nodeParams
                    );
                    ...
                    if (cyTarget.isNode && cyTarget.isNode()) {
                        if (
                            cyTarget.data('class').startsWith('complex') ||
                            cyTarget.data('class') === 'compartment' ||
        ...
        cy.on('unselect', 'node', function () {
            if (
                ...
                appUtilities.firstSelectedNode.id() === this.id()
            ) {
        ...
        cy.on('layoutstart', function (event) {
            ...
            if (node.length == 1) node[0].data('onLayout', true);
        });
        ...
        cy.on('layoutstop', function (event) {
            ...
            cy.nodes('[?onLayout]').forEach(function (node) {
                node.removeData('onLayout');
            });
        });
