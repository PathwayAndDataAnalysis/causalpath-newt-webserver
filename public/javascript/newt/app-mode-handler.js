...
const modeHandler = {
	...
	setAddNodeMode: function (nodeType, language, _cy) {
		...
		const typeChange = nodeType && nodeType !== modeProperties.selectedNodeType; // See if the type will change
	...
	setAddEdgeMode: function (edgeType, language, _cy) {
		...
		const typeChange = edgeType && edgeType !== modeProperties.selectedEdgeType; // See if the type will change
	...
	setPreviousMode: function () {
		...
		} else if (modeHandler.perviousMode == 'marquee-zoom-mode') {
			//marquee zoom mode
